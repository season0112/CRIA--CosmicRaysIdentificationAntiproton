/*
 * Plot paper2016 and whole time range comparison
 * Plot Raw and unfolded result
 * Plot chi2 plot
 */


int Plot_Low_AveragedRatio(){
...
c1.Update();
c1.SaveAs( "paper2016andpass78_compare.pdf" );


//// Plot Unfolding and Raw result
TCanvas c4("c4","c4",1000,500);

unfolded_2016->SetMarkerStyle(15);
unfolded_2016->SetMarkerColor(2);
raw_2016->SetMarkerStyle(15);
raw_2016->SetMarkerColor(8);

gPublishedRatio->Draw("AP");
raw_2016->Draw("same P");
unfolded_2016->Draw("same P");
g_IntermediateResult_2016->Draw("same P");

TLegend * leg4 = new TLegend(0.2,0.65,0.5,0.85); //(xmin, ymin, xmax, ymax)
leg4->SetFillColor(0);
leg4->AddEntry(gPublishedRatio,"PRL paper","lp");
leg4->AddEntry(unfolded_2016,"Unfolded (Published Range)","lp");
leg4->AddEntry(raw_2016,"Raw (Published Range)","lp");
leg4->AddEntry(g_IntermediateResult_2016,"TRD fit (Published Range)","lp");
leg4->Draw();

c4.Update();
c4.SaveAs( "RawAndUnfolded_Result.pdf" );
...
g_chi2_tof_pass78->Draw("AP");

g_chi2_tof_pass78->SetMarkerStyle(15);
g_chi2_tof_pass78->SetMarkerColor(1);
TAxis * xaxis_chi2_tof_pass78 = g_chi2_tof_pass78->GetXaxis();
TAxis * yaxis_chi2_tof_pass78 = g_chi2_tof_pass78->GetYaxis();
