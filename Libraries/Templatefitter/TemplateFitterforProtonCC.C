...
#define INFO_OUT_TAG "TemplateFitterforProtonCC"
...
double TemplateFitter::ExecuteFit(void (*function)(int&, double*, double&, double*, int)) {
  ...
  fMinimizer->SetPrintLevel(fPrintLevel); //set Minuit print level. -1:quiet, 0:normal, 1:verbose
  fMinimizer->SetFCN(function); // The MINUIT package acts on a multiparameter Fortran function to which one must give the generic name FCN. Example: likelihood fun.
  ...
  for (unsigned int i = 0; i < fInputTemplates.size(); i++) {
    ...
    // mnparm: Implements one parameter definition. Called from MNPARS and user-callable Implements one parameter definition.
  ...
  for (unsigned int i = 0; i < fInputTemplates.size(); ++i) {
    if (!fFixParameter.at(i))
      continue;
    // mnfixp:Removes parameter IINT from the internal parameter list, and arranges the rest of the list to fill the hole.
  ...
  // mnexcm: Interprets a command and takes appropriate action. mnexcm( const char * command,  Double_t * plist,  Int_t llist,  Int_t & ierflg )       
  // Minimization method: MIGRAD, 
  // Hesse algorithm to compute asymptotic errors.
  // Minos algorithm to compute confidence intervals.
  ...
  // mnemat: Calculates the external error matrix from the internal matrix.
  ...
  for (unsigned int k = 0; k < fInputTemplates.size(); ++k) {
    ...
    // mnerrs: Utility routine to get MINOS errors.
    fMinimizer->mnerrs(k, errorUp, errorLow, error_parabola, correlation); //error_parabola is 'parabolic' error (from error matrix). (Errors not calculated are set = 0), correlation is global correlation coefficient from error matrix.
...
void TemplateFitter::llh_fit_func(int&, double*, double& f, double* par, int) {
  ...
  for (unsigned int i = 0; i < list->vec[index_data_data].size(); i++) {
    ...
    if (TempSum != 0) { //only use points for which template information exists
      for (int k = 0; k < NumberOfTemplates; k++)
        logi += par[k] * list->vec[index_data_data + 1 + k].at(i) * list->vec[index_Nentries].at(0) * list->vec[index_BinWidth].at(i); // m_i = par[k] * list->vec[index_data_data + 1 + k].at(i) * list->vec[index_Nentries].at(0) * list->vec[index_BinWidth].at(i)

      if (logi <= 0.0)
        logi = 1.e-10;

      logi = std::log(logi);
      LogL += list->vec[index_data_data].at(i) * list->vec[index_Nentries].at(0) * list->vec[index_BinWidth].at(i) * logi;  //n_i = list->vec[index_data_data].at(i) * list->vec[index_Nentries].at(0) * list->vec[index_BinWidth].at(i) 
    }
  }
  f = -2 * (LogL - m_tot); // -log(Llh) = - 2 * (Sum(n_i*log(m_i))-m_tot)  ( times 2 doesn't matter )
}
