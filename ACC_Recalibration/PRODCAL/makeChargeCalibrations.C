...
//   std::cout<< "tree" << tree <<std::endl;
   ...
   gg2_1x->Draw("same");
   czeta->SaveAs(Form("png_cal/QTimecal2_%d_1.png",ips));
   czeta->SaveAs(Form("png_cal/QTimecal2_%d_1.root",ips));
   ...
   gg2_0x->Draw("same");
   czeta->SaveAs(Form("png_cal/QTimecal2_%d_0.png",ips));
   czeta->SaveAs(Form("png_cal/QTimecal2_%d_0.root",ips));

   TH2D* gg1_1 = new TH2D("gg1_1","gg1_1",365*ny,t0,24*3600*365*ny+t0,200,0,6.9);
   SetXaxisTimeHist(gg1_1);
   tree->Draw("uncal_qside1:run>>gg1_1",Form("(qtot<1.2 && qtot>0.8) && sector==%d",ips) && evclean,"colz");   
   TH1D* gg1_1x = (TH1D*) gg1_1->ProjectionX("gg1_1x");
   FindMaxX(gg1_1,gg1_1x,0,0,6,10.);
   SliceNormalizeX(gg1_1,-1,(TGraph*)1,(TGraph*)1,0,0);
   gg1_1->Draw("colz");
   gg1_1->GetYaxis()->SetTitle("ACC side 1 charge for P");
   gg1_1x->Draw("same");
   czeta->SaveAs(Form("png_cal/QTimecal1_%d_1.png",ips));
   czeta->SaveAs(Form("png_cal/QTimecal1_%d_1.root",ips));

   TH2D* gg1_0 = new TH2D("gg1_0","gg1_0",365*ny,t0,24*3600*365*ny+t0,200,0,5.9);
   SetXaxisTimeHist(gg1_0);
   tree->Draw("uncal_qside0:run>>gg1_0",Form("(qtot<1.2 && qtot>0.8) && sector==%d",ips) && evclean,"colz");   
   TH1D* gg1_0x = (TH1D*) gg1_0->ProjectionX("gg1_0x");
   FindMaxX(gg1_0,gg1_0x,0,0,6,10.);
   SliceNormalizeX(gg1_0,-1,(TGraph*)1,(TGraph*)1,0,0);
   gg1_0->Draw("colz");
   gg1_0->GetYaxis()->SetTitle("ACC side 0 charge for P");
   gg1_0x->Draw("same");
   czeta->SaveAs(Form("png_cal/QTimecal1_%d_0.png",ips));
   czeta->SaveAs(Form("png_cal/QTimecal1_%d_0.root",ips));
   
   TFile mfout(Form("OUTCAL/QTimecal%d.root",ips),"recreate");
   gg2_1x->Write(Form("Qcal2_%d_1",ips));
   gg2_0x->Write(Form("Qcal2_%d_0",ips));
   gg1_1x->Write(Form("Qcal1_%d_1",ips));
   gg1_0x->Write(Form("Qcal1_%d_0",ips));
   mfout.Close();
