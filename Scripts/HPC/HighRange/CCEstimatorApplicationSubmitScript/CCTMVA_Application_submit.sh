...
do
    mkdir -p p$pattern/PhyRep2021
    cd p$pattern/PhyRep2021
    for binname in $bin
    do
        echo $binname
        mkdir -p $binname
        cd $binname
        run_parallel_generic_job --argument "--rigidityBinSection $binname --issversion PhyRep2021 --trdProtonHeliumCut --physicstriggerCut --pattern $pattern" --timelimit 24:00 --memory 180 --partition c18m /home/bo791269/Software/AntiprotonAnalysis/Scripts/HPC/HighRange/TMVA_Application_submit.py 0 0
        cd ..
    done
    cd ../..
done
