...
reader.BookMVA("MLPBNN","dataset_pymva/weights/TMVAClassification_MLPBNN.weights.xml")
########################################################################################
...
mva_MLPBNN_signal_array = np.array([])
mva_MLPBNN_background_array = np.array([])
...
for ievt in range(signal_array.shape[0]):
	...
	mva_MLPBNN = reader.EvaluateMVA("MLPBNN")
	mva_MLPBNN_signal_array = np.append(mva_MLPBNN_signal_array,mva_MLPBNN)      
	print("ievt="+str(ievt))
	print("mva_MLPBNN="+str(mva_MLPBNN))
np.save("application_results/mva_MLPBNN_signal_array.npy", mva_MLPBNN_signal_array)
...
for ievt in range(background_array.shape[0]):
        ...
        mva_MLPBNN = reader.EvaluateMVA("MLPBNN")
        mva_MLPBNN_background_array = np.append(mva_MLPBNN_background_array,mva_MLPBNN)
        print("ievt="+str(ievt))
        print("mva_MLPBNN="+str(mva_MLPBNN))
np.save("application_results/mva_MLPBNN_background_array.npy", mva_MLPBNN_background_array)
