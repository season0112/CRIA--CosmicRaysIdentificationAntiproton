...
######################### Test events ##################################
testset1 = np.load('../data/B1042_pr.pl1.flux.l1o9.2016000_7.6_all_positive_147_175_pattern_0.npy',encoding="latin1")
tem = np.load('../data/B1042_pr.pl1.flux.l1a9.2016000_7.6_all_negative_147_175_pattern_0.npy',encoding="latin1")
testset2 = tem[-int(tem.shape[0]*0.2),:]
testset2 = np.row_stack((testset2,np.load('../data/B1042_antipr.pl1.1800_7.6_all_positive_147_175_pattern_0.npy',encoding="latin1")))
testset2 = np.row_stack((testset2,np.load('../data/B1042_pr.pl1.flux.l1o9.2016000_7.6_all_negative_147_175_pattern_0.npy',encoding="latin1")))
...
testset[:,4] = testset[:,4] * sign_test
testset[:,5] = testset[:,5] * sign_test
testset[:,6] = testset[:,6] * sign_test
features = 16

##############  model prediction   #################
import CNN_models
model = CNN_models.VGG16(features)
model.load_weights('VGG16_147_175.h5')
...
for CCprotoniterate in range(binnumber):
        ...
        if CCprotonaccumulatecount == 0 :
            CCprotonaccumulatecount = 0.0000000001
...
for protoniterate_LikelihoodKDE in range(binnumber):
        protonaccumulatecount_LikelihoodKDE=protonaccumulatecount_LikelihoodKDE+np.histogram(LikelihoodKDE_signal,binnumber,range=(0,1))[0][binnumber-protoniterate_LikelihoodKDE-1]
...
for CCprotoniterate_LikelihoodKDE in range(binnumber):
        CCprotonaccumulatecount_LikelihoodKDE=CCprotonaccumulatecount_LikelihoodKDE+np.histogram(LikelihoodKDE_background,binnumber,range=(0,1))[0][binnumber-CCprotoniterate_LikelihoodKDE-1]
...
plt.plot(fractionoprotonset_SVM, rejectionCCprotonset_SVM, 'y-', lw=3,label='SVM')
plt.plot(fractionoprotonset_LikelihoodKDE, rejectionCCprotonset_LikelihoodKDE, 'c-', lw=3,label='LikelihoodKDE')
plt.plot(fractionoprotonset_Fisher, rejectionCCprotonset_Fisher, 'm-', lw=3,label='Fisher')
plt.plot(fractionoprotonset_Likelihood, rejectionCCprotonset_Likelihood, 'r-', lw=3,label='Likelihood')
