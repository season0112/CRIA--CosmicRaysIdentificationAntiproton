...
reader.BookMVA("Fisher","dataset_pymva/weights/TMVAClassification_Fisher.weights.xml")
########################################################################################
...
mva_Fisher_signal_array = np.array([])
mva_Fisher_background_array = np.array([])
...
for ievt in range(signal_array.shape[0]):
	...
	mva_Fisher = reader.EvaluateMVA("Fisher")
	mva_Fisher_signal_array = np.append(mva_Fisher_signal_array,mva_Fisher)      
#	print("ievt="+str(ievt))
#	print("mva_Fisher="+str(mva_Fisher))
np.save("application_results/mva_Fisher_signal_array.npy", mva_Fisher_signal_array)
...
for ievt in range(background_array.shape[0]):
        ...
        mva_Fisher = reader.EvaluateMVA("Fisher")
        mva_Fisher_background_array = np.append(mva_Fisher_background_array,mva_Fisher)
#        print("ievt="+str(ievt))
#        print("mva_Fisher="+str(mva_Fisher))
np.save("application_results/mva_Fisher_background_array.npy", mva_Fisher_background_array)
