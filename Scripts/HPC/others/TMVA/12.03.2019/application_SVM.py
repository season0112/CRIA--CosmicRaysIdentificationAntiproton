...
reader.BookMVA("SVM","dataset_pymva/weights/TMVAClassification_SVM.weights.xml")
########################################################################################
...
mva_SVM_signal_array = np.array([])
mva_SVM_background_array = np.array([])
...
for ievt in range(signal_array.shape[0]):
	...
	mva_SVM = reader.EvaluateMVA("SVM")
	mva_SVM_signal_array = np.append(mva_SVM_signal_array,mva_SVM)      
	print("ievt="+str(ievt))
	print("mva_SVM="+str(mva_SVM))
np.save("application_results/mva_SVM_signal_array.npy", mva_SVM_signal_array)
...
for ievt in range(background_array.shape[0]):
        ...
        mva_SVM = reader.EvaluateMVA("SVM")
        mva_SVM_background_array = np.append(mva_SVM_background_array,mva_SVM)
        print("ievt="+str(ievt))
        print("mva_SVM="+str(mva_SVM))
np.save("application_results/mva_SVM_background_array.npy", mva_SVM_background_array)
