...
def drawCompareWithElectron(rigidityrange, lowworkpath, intermediateworkpath, lepton_result, lepton_result_Maura, trackerpattern, richcut, plotrange, binmerge, Electron, Positron, Electron_error_relative, Positron_error_relative, yaxisfactor, index, mode, ElectronPositronIndex):
    ...
    plt.errorbar(dates_1Bartel_Maura, EleOverPosRatio_Rebin_Perugia * Electron_Scaler, yerr=EleOverPosRatioError_Rebin_Perugia * Electron_Scaler, marker='s', linestyle="None", markersize=15, markerfacecolor="red",ecolor="red", label=r'$\rm{e^-}$/$\rm{e^+}$' +  r' $\times$ ' + "{:.2}".format(Electron_Scaler) )  #label=r'$\rm{e^-}$/$\rm{e^+}$' +  r' $\times$ ' + "{:.5}".format(Electron_Scaler), label='_nolegend_'
    ...
    ax1.tick_params(axis='y', labeltop=False,labelbottom=True, labelleft=True, labelright=True, colors='red', direction='in', length=12, width=10)
    ...
    if mode == "6months":
        ...
        ax2.errorbar(dates[0:15]+dates[16:38]+dates[39:], np.delete(B3result,[15,38]) * Antiprotonratio_Scaler, yerr=np.delete(Error3b,[15,38]) * Antiprotonratio_Scaler, marker='o',linestyle="None",markersize=25, markerfacecolor="b", ecolor="b", label= r'$\overline{\rm {p}}$/p $\times$ 10$^{-5}$' )
    elif mode == "6Bartels":
